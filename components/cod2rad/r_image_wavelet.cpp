...
static WaveletHuffmanDecode* waveletDecodeBlue = (WaveletHuffmanDecode*)0x0045AD30;
static WaveletHuffmanDecode* waveletDecodeRedGreen = (WaveletHuffmanDecode*)0x0045ED30;
static WaveletHuffmanDecode* waveletDecodeAlpha = (WaveletHuffmanDecode*)0x00462D30;
...
static void Wavelet_ConsumeBits(unsigned __int16 bitCount, WaveletDecode *decode)
{
	...
	decode->value |= ((*(decode->data + 3) << 24) | (*(decode->data + 2) << 16) | (*(decode->data + 1) << 8) | *decode->data) >> decode->bit << (16 - bitCount);
	decode->bit += bitCount;
...
static void Wavelet_DecompressLevel(char *src, char *dst, WaveletDecode *decode)
{
	...
	"decode->bpp not in [1, 4]\n\t%i not in [%i, %i]",
		...
		4
	*/
	ASSERT(decode->bpp >= 1 && decode->bpp <= 4);
	...
	if (w > 1 && h > 1)
	{
		...
		{
			decode->value = (*(decode->data + 1) << 8) | *decode->data;
			decode->bit = 0;
			decode->data += 2;
		...
		for (int y = 0; y < h; y += 2)
		{
			for (int x = 0; x < w; x += 2)
			{
				...
				if (decode->channels != 1)
				{
					...
					int base = 2 * src[dstChanOffset[0]];
					...
					if (decode->channels >= 3)
					{
						...
						coeff[1][0] = coeff[0][0] + Wavelet_DecodeValue(waveletDecodeRedGreen, 0xAu, 510, decode);
						coeff[1][1] = coeff[0][1] + Wavelet_DecodeValue(waveletDecodeRedGreen, 0xAu, 510, decode);
						coeff[1][2] = coeff[0][2] + Wavelet_DecodeValue(waveletDecodeRedGreen, 0xAu, 510, decode);
						...
						coeff[2][0] = coeff[0][0] + Wavelet_DecodeValue(waveletDecodeRedGreen, 0xAu, 510, decode);
						coeff[2][1] = coeff[0][1] + Wavelet_DecodeValue(waveletDecodeRedGreen, 0xAu, 510, decode);
						coeff[2][2] = coeff[0][2] + Wavelet_DecodeValue(waveletDecodeRedGreen, 0xAu, 510, decode);
				...
				else
				{
					...
					// TODO - Validate this
					//int base = 2 * src[*(&v9 + decode->channels)];
					//char* dstChan = &dst[*(&v9 + decode->channels)];
					int base = 2 * src[dstChanOffset[decode->channels]];
					char* dstChan = &dst[dstChanOffset[decode->channels]];
...
void __cdecl Image_DecodeWavelet(GfxRawImage *image, t5::GfxImageFileHeader *imageFile, unsigned __int8 *data, int bytesPerPixel)
{
	...
	for (decode.data = (const char *)data; decode.mipLevel >= 0; --decode.mipLevel)
	{
